...
export function useForm<T extends z.ZodType<any, any>>({
  ...
  const setFieldError = React.useCallback(
    (path: string, errorMessage: string) => {
      setErrors((prev) => {
        return setValueByPath(prev, path, { message: errorMessage });
      });
    },
    [],
  );
  ...
  const validateForm = React.useCallback((): boolean => {
    ...
    try {
      ...
      if (err instanceof ZodError) {
        ...
        const tree: FormErrors = {};
        validationErrors.forEach((issue: ZodIssue) => {
          const path = issue.path.join(".");
          const errorMessage = issue.message; // ZodIssue.message is always string
          // build nested tree
          setValueByPath(tree, path, { message: errorMessage });
        });
        setErrors(tree);
        ...
        if (!result.success) {
          ...
          result.error.errors.forEach((issue) => {
            ...
            if (shouldIncludeError) {
              next = setValueByPath(next, ip, { message: issue.message });
            }
          });
        }
        return next;
...
type FormErrors = {
  [key: string]: FormError | FormErrors;
};

type UseFormReturn<T extends z.ZodType<any, any>> = {
  values: z.infer<T>;
...
type FormContextValue<T> = Omit<UseFormReturn<T>, "handleSubmit">;
...
function getValueByPath(obj: any, path: string): any {
  ...
  for (const key of keys) {
    ...
    if (typeof current !== "object" || Array.isArray(current)) {
      // If current is not an object (e.g., a string error message) or is an array,
      // we can't traverse further using object keys.
      return undefined;
    }
    current = current[key];
  }
...
function deleteValueByPath(
  ...
  if (pointer && typeof pointer === "object" && last in pointer) {
    if (options?.shallow) {
      ...
      if (typeof pointer[last] === "object" && "message" in pointer[last]) {
        delete pointer[last].message;
        // If the object becomes empty after removing the message, delete the object itself
        if (Object.keys(pointer[last]).length === 0) {
          delete pointer[last];
        }
      }
    } else {
